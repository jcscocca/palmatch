...
- [x] **From F3 spec review:** bound decodeOwnedShare's decompressed output (hostile `#/own/` link can inflate ~12MB on the main thread today) — extract a shared bounded-inflate helper (src/lib/) used by both the share codec and wrapper.ts's inflateOnce, or cap via pako streaming with limit; add truncated-deflate-stream tamper case.
- [x] **Carried from F2 quality review + F3 report:** split `nonPalRows` → `playerRows` + `unreadableRows` in parse.ts/ImportResult (3 lines; lets the summary say "guild of N players" honestly); add structured `oddTypes: string[]` + `unknownPals: number` to ImportResult (warnings prose stays as convenience); export ONE shared `buildLowerLookup` helper (url.ts, parse tests, worker client, panel all hand-roll it); worker: hoist `requestId = req?.requestId ?? -1` above the try (undefined event.data currently → unhandled rejection); window-level dragover/drop guard that auto-opens the import panel (page-drop currently navigates the tab away); `closeCharacterMap` helper; codeOf/detailOf shared test util; builder mirror-note names its uncovered surface (wrapper+headers).
- [x] **Spec amendment (decided during F3):** the "decompressing → scanning" progress line is CUT — parses complete in seconds; the indeterminate pulse + "READING N MB" line is the shipped behavior. Terminal-only worker protocol stands.
- [x] README: MY PALS section (how import works, privacy line — save never leaves the browser; share-link explanation; Xbox limitation; troubleshooting table from ParseError codes).
- [x] Import-panel a11y pass (labels, focus flow — mirror SearchPalette patterns) + mobile layout (drop zone full-width, hints collapsible).
- [x] Bundle check: `npm run build` → verify ooz-wasm is a separate lazy chunk not in the entry; note sizes in the task report.
- [x] Full gates + `npm run data:refresh` idempotence still green. Commit: `chore: save-import polish and docs`
- [x] **Verification-review fixes (second F5 pass):** ingest generation counter closing the `arrayBuffer()` read window (a share dropped mid-read was clobbered by the save's continuation — abortInFlight only covers the post-`postMessage` half); `worker.onerror` matched on request id like `onmessage`; `new Worker(...)` wrapped in try/catch → coded `internal` (CSP/no-module-worker browsers wedged the panel on READING); MY PALS badge counted through the dataset filter so it can't disagree with the panels' `hasOwnedFor`; "guild of N players" only above 1. Plus: `isSafeInteger` in both count validators, sticky import-panel head, `codeOfSync` in wrapper.test, stale comments corrected. Commit: `fix: close import race windows per verification review`

**F5 shipped sizes** — entry 289.73 kB → 249.43 kB raw (90.97 → 78.33 kB gzip); share codec split to its own lazy `owned-share` chunk (43.97 kB / 13.76 kB gzip), reached only when the import dialog opens. Note for future readers: conditional *mounting* does not split a chunk — `Workbench` imports `ImportPanel` eagerly, so the split comes from a dynamic `import()` of the codec inside the panel.
